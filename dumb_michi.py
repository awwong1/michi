...
def gtp_io():
    ...
    while True:
        ...
        if command[0] == "boardsize":
            ...
            # Dumb Michi does not care about Komi
            pass
        elif command[0] == "play" and state[-1]:
            ...
            if c is not None:
                ...
                pass
        elif command[0] == "genmove" and state[-1]:
            x, y = random.randint(1, N), random.randint(1, N)
            possible_moves = list(state[-1].moves(y * W + x))
            if state[-1].last is None:
                state.append(state[-1].pass_move())
                ret = 'pass'
            elif len(possible_moves) > 0:
                possible_move = possible_moves[0]
                state.append(state[-1].move(possible_move))
                if state[-1]:
                    ret = str_coord(state[-1].last)
                else:
                    ret = 'pass'
            else:
                ret = 'resign'
        elif command[0] == "final_score":
            # Final Score isn't functional, it will spit out garbage
            back_step = -1
            score_state = state[back_step]
            while score_state is None:
                back_step -= 1
                score_state = state[back_step]
